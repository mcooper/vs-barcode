...
def replaceVars(df):
    ...
    idx = ~(df['name'].isnull() | df['DB_Vars'].isnull())
    for i in df.index[idx]:
        df = df.replace({'\{' + df.loc[idx, 'name'][i] : '{' + df.loc[idx, 'DB_Vars'][i]}, regex=True)
...
metadata_definition = [
    ['User_Vars',           'The name of the variables in the views that users download as parsed from the view definition scripts'],
    ['User_Tables',         'The name of the views that users download, as parsed from the view definition scripts'],
    ['DB_Vars',             'The name of the variable in the database'],
    ['DB_Tables',           'The name of the table in the database'],
    ...
    ['constraint_message',  'From ODK: The text displayed to data collectors when a variable violates the constraint condition'],
...
include_cols = ['DB_Vars', 'DB_Tables', 'label', 'required', 'relevant', 'constraint', 'constraint_message', 'values']

#To Do Map table names to names in the download tool

for i in userdf['DB_Tables'].dropna().unique():
    writer = ps.ExcelWriter('metadata_summaries/' + i + '.xlsx')
    userdf.loc[userdf['DB_Tables']==i,include_cols].drop_duplicates().to_excel(writer, i[:31], index=False)
